...
- 扫描次数：整个过程会扫描1次小表B的聚簇索引，扫描出100行。扫描1次大表A的聚簇索引，扫描出10000行。join_buffer里面的数据是无序的，大表A中每次取出一行来和join_buffer判断的时候，内存都得做100次判断，所以内存的判断次数是100*10000=1百万。如果改成用NLJ算法，回到磁盘扫描1百万次，而BNL只需要磁盘扫描100100次+内存判断1百万次，内存判断速度很快。
