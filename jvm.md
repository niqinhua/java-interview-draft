...
- 后来就有了多线程的垃圾回收器，就是新生代的Parallel Scavenge 和老年代的 Parallel Old，垃圾回收时是多个GC线程的，也得暂停用户线程。这种一般适合一百兆到几个G的堆空间。这种垃圾回收器关注的是吞吐量，jdk8默认也是这一套。
- 但是如果你想缩短响应时间的话，就可以用CMS老年代垃圾回收器，它可以搭配单线程的Serial和多线程的ParNew这两种新生代垃圾回收器。由于原来的多线程Parallel Scavenge这个新生代垃圾回收器没法兼容CMS老年代垃圾回收期，所以才单独创建了功能都差不多的ParNew。他一般适用于几个G到几十G的堆空间。
CMS的流程就是先暂停用户线程来标记所有可以作为GCRoot 的对象，因为 GCRoot对象很少，所以这里暂停时间很短，这一步叫做初始标记。然后用户线程和GC线 程就可以一起跑了，然后GC根据GCRoot对象的扫描所有对象找到可回收的对象，这一步
是花费最长的时间，叫做并发标记。要注意的一点就是GC线程边找垃圾对象，用户线程可 能还会对对象的引用进行修改，就是可能会发生漏标问题，就是会产生一些漏标的垃圾对 象，而且用户线程还会边产生更多垃圾，所以还得暂停一下用户线程，来重新收集一下还没 标记到的垃圾，这一步由于还没标记的垃圾一般不多，所以不会暂停用户线程很久，这一步 叫做重新标记。然后最后用户线程和GC线程再一起跑，GC就去清除掉这些垃圾，这一步叫 做并发清除。
CMS的缺点最关键的就是有浮动垃圾，因为最后一步并发清除，GC线程边回收垃 圾，用户线程边产生垃圾。而且由于浮动垃圾，GC之前必须预留这部分空间，而且很恐怖 的就是，如果浮动垃圾太多，就直接暂停所有线程，用Serial old替代来执行FullGC。第二 点就是因为用户线程和GC线程一起跑，只能用标记清除算法，那肯定就会有内存碎片，如 果因为没有连续空间存不下对象，也会用Serial old来替代做标记整理清除算法。第三点就 是因为并发的GC线程，CPU一般要4核以上比较好;
G1: CMS没有办法控制暂停用户线程 的时间，如果想要尽量控制可以用 G1。它一般适用于几十G到上百G的堆空间。G1就是将堆划分成很多个相等大小的 region，每个region可以是eden区，幸存者区，也可以说老年区，还有一个专门大 对象的区域，你也可以直接当做老年区来看。它最大特点，就是在标记垃圾之后，可 以根据我们期望的暂停用户线程的时间来挑选出最有回收价值的region来回收。
G1的流程就是先暂停用户的线程来标记所有可以作为GCRoot的对象，因为 GCRoot对象很少，所以这里暂停时间很短，然后还有修改两个TAMS的指针，这两个指针 就是划分出后面用户线程和GC线程一起跑的时候，用户线程新创建的对象就存到这两个指 针之间的空间，这一步叫做初始标记。然后用户线程和GC线程就可以一起跑了，然后GC根 据GCRoot对象的扫描所有对象找到可回收的对象，这一步是花费最长的时间，叫做并发标 记，要注意的一点就是GC线程边找垃圾对象，用户线程可能还会对对象的引用进行修改， 就是可能会发生漏标问题，漏标问题等下说，就是会产生一些漏标垃圾的对象，所以还得暂 停一下用户线程，来重新标记一下这些漏标的垃圾对象，这一步由于还没标记的垃圾一般不 多，所以不会暂停用户线程很久，这一步叫做最终标记。然后GC就去统计哪些region最有 回收价值，根据设置的期望用户线程暂停时间的值来设置回收计划，把region活的对象赋 值到别的地方去，然后再清理掉整个region，这一步由于要移动对象，所以用户线程是必 须暂停的，这一步叫做筛选回收。
