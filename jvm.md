...
- 说下类加载器
    ...
    - 当然你也可以自定义类加载器，就是自己去写个类去继承抽象类ClassLoader，你可以去重写loadClass方法打破双亲委派，也可以不重写保留他原先的双亲委派机制。然后主要是得重写findClass方法来控制自定义类加载器扫描扫描的类路径。另外，除了引导类加载器以外，其他类加载器都得在parent的属性变量里面配置父类加载器，抽象类ClassLoader默认是父类加载器是应用类加载器。
...
- 内存空间初始化：将分配到的内存空间都初始化为零值（不包括对象头）， 如果使用TLAB，这一工作过程也可以提前至TLAB分配时进行。
- 设置对象的对象头：然后设置对象的对象头。（展开描述参考[对象头](#ObjetHeader)）
- 对象的初始化：然后才会进行属性赋值、执行构造方法。
...
- 一般情况就是一个对象的分代年龄满15次就会放到old区（长期存活的对象进入老年代），但也有特殊情况，比如CMS收集器默认6岁，还比如就是年龄1+年龄2+年龄n的多个年龄对象总和超过了幸存者区的一半，此时就会把年龄大于等于n以上的对象都放入老年代。这个规则其实是希望那些可能是长期存活的对象，尽早进入老年代。（对象动态年龄判断机制）。
...
- 可达性分析：就是先确定所有叫做GCRoot的对象作为起始节点，向下一层一层搜索引用关系，如果一个对象到GCRoot没有任何引用链，就说明这个对象是垃圾。
    ...
    - 可以作为GCRoot的对象包括：虚拟机栈中的局部变量表中引用的对象、本地方法栈中的引用对象、方法区的静态变量和字符串常量池引用的对象、 被synchronize持有的对象、jvm的内部引用(比如系统类加载器、class对象、异常 类对象)
...
- 软引用就是GC做完后发现释放不出空间存放新的对象，则会把这些软引用的对象回收掉。（使用场景：浏览器的后退按钮） 
...
### 垃圾回收算法
- 第一个就是复制算法，就是把内存分成两块，一边存满了，就先GC，还活着的对象或者新增的对象就都复制到另外一边，刚刚那一边就整块一起清空掉，又满了，就又复制另外一边。他这种复制得越少效率就越高，所以他适合用于新生代，新生代能活的对象并不多。但是这种分成两块的浪费了一大半空间，所以它有一个优化版本，就是分成8:1:1。这种就只需要浪费10分之一的空间而已。大部分的新生代垃圾回收器都是用这种算法。
...
- 如果你想缩短响应时间话，就可以用CMS老年代垃圾回收器，因为可以缩短GC停顿时间的。它可以搭配单线程的Serial和多线程的ParNew这两种新生代垃圾回收器。他一般适用于几个G到几十G的堆空间。
    - 初始标记：CMS的流程就是先暂停用户线程来标记所有可以作为GCRoot 的对象，因为 GCRoot对象很少，所以这里暂停时间很短，这一步叫做初始标记。
    - 并发标记：然后垃圾回收器根据GCRoot对象的扫描所有对象找到可回收的对象，这一步是GC里花费时间最长的，但不会阻塞用户线程，GC线程和用户线程一起跑的，这一步叫做并发标记。
    - 重新标记：要注意的一点就是GC线程边找垃圾对象，用户线程可能还会对对象的引用进行修改，会产生一些漏标的垃圾对象，而且用户线程还会边产生更多垃圾，所以还得暂停一下用户线程，来重新收集一下还没标记到的垃圾，这一步由于还没标记的垃圾一般不多，所以不会暂停用户线程很久，这一步叫做重新标记。
    - 并发清除：然后最后用户线程和GC线程再一起跑，GC就去清除掉这些垃圾，这一步叫做并发清除。CMS的缺点最关键的就是有浮动垃圾，因为并发清除时，GC线程边回收垃圾，用户线程边产生垃圾，所以由于这些浮动垃圾，GC之前必须预留这部分空间，而且要注意的就是，如果浮动垃圾太多，就直接暂停所有线程，老年代用Serial old替代来执行FullGC。第二点就是因为用户线程和GC线程一起跑，只能用标记清除算法，那肯定就会有内存碎片，如果因为没有连续空间存不下对象，也会用Serial old来替代做标记整理算法。第三点就是因为并发的GC线程，CPU一般要4核以上比较好;
    - 并发重置：最后会重置本次GC过程中的标记数据。

![image](https://user-images.githubusercontent.com/27798171/182285506-b997d3fc-fdf9-4160-879a-25dd4e349b30.png)

- G1: CMS没有办法控制暂停用户线程 的时间，如果想要尽量控制可以用 G1。它一般适用于几十G到上百G的堆空间。G1就是将堆划分成很多个相等大小的 region，每个region可以是eden区，幸存者区，也可以是老年区，还有一个专门大对象的区域，你也可以直接当做老年区来看。它最大特点，就是在标记垃圾之后，可以根据我们期望的暂停用户线程的时间来挑选出最有回收价值的region来回收。
...
![image](https://github.com/niqinhua/java-interview-draft/assets/27798171/f94a1253-c717-4a74-9cd9-a6528ad4bed3)
