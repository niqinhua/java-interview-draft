...
- aof
  ...
  - 如果redis已经跑了一年，那这个aof文件会非常庞大，恢复数据会特别慢，比如一个key如果incr了好几千次，没必要记录几千次，只需要记录最后的值就好，所以需要对aof进行重写。


- aof重写
  - bgrewriteaof指令用于对AOF日志进行重写，原理就是主进程fork一个子进程，子进程扫描内存的所有数据，转换为Redis的操作指令，再以resp协议覆盖原来的aof文件。
