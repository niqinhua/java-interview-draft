...
-  rdb
 -  可以通过save指令生成rdb快照，但是saven是在主进程执行的会阻塞客户端命令。所以redis默认是用bgsave指令生成rdb快照，在配置文件可以配置一定时间内有多少个key有改动就触发bgsave。
 -  bgsave的流程就是：redis会在主进程fork一个子进程，子进程可以共享主进程的所有内存数据，由子进程执行持久化。
