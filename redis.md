...
- 脑裂数据丢失问题：redis哨兵模式或集群模式没有过半机制就会有脑裂问题，脑裂问题就是某一个主节点因为网络问题或者是正在处理大量客户端请求比较繁忙导致心跳回复失败，于是触发了主从切换机制选了新的主节点。在切换过程中，原来的主节点还在和部分客户端通讯，导致两个主节点同时在处理请求。危害就是比如：setNX可能会拿到相同的锁；基于incr生成的全局唯一id，也可能出现重复。当后面原来的主节点发现已经有新的主节点了，就会清空本地数据，开始全量同步新master数据，导致在双主的过程中，向原来主节点请求的的数据没了。所以最好设置写数据的时候，要同步一半的从节点才算写成功。不过这样会影响集群的可用性和性能，一般也不会配置。
```
